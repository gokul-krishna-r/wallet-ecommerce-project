...
router.post("/checkout", middleware.isLoggedIn, async (req, res) => {
  ...
  const userDetail=await User.find({_id:req.user})
  if(userDetail[0].credit >= cart.totalCost){
  const order = new Order({
        ...
        order.save(async (err, newOrder) => {
          ...
          req.session.cart = null;
          console.log("=--------")
          User.findOneAndUpdate({_id:req.user},{$inc:{credit:-cart.totalCost}},function(err, result) {
            console.log("222.=--------")

            if (err) {
              console.log('Error decreasing user credit', err);
            } else {
              console.log('User credit decreased successfully');
            }})
        ...
        res.render("user/profile", {
          orders: allOrders,
          errorMsg:"Purchase Failed. Not enough balance",
          successMsg:"",
          pageName: "User Profile",
          user:userDetail[0].credit,
        });
      }
